...
- The second column should have the header `Value` and include the metadata values: C, `2018-2019`, and `EPA Air Quality System (AQS)`.
...
```{r boxplot 1, echo=FALSE, fig.align='left', fig.height=2.8, fig.cap='Ozone Levels (2018)'}

Ozone2018Plot <- epa_data %>%
  filter(Year == 2018) %>%
  drop_na(Ozone) %>%
  ggplot() + 
  aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone) +
  geom_boxplot() +
  labs(x = 'Month', y = expression(paste("Ozone (", mu,"g/", m^{3}, ")")))

print(Ozone2018Plot)

```

```{r boxplot 2, echo=FALSE, fig.align='left', fig.height=2.8, fig.cap='Ozone Levels (2019)'}

Ozone2019Plot <- epa_data %>%
  filter(Year == 2019) %>%
  drop_na(Ozone) %>%
  ggplot() + 
  aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone) +
...
## Git Repository
[alex-lopez70](https://github.com/alex-lopez70/)